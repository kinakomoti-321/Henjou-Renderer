	...
	//GeometryData
	{
		...
		for (auto& nodes : model.nodes) {
			...
			if (nodes.mesh != -1) {
				...
				GeometryData gasdata;
				gasdata.index_offset = scenedata.indices.size();
				scenedata.prim_offset.push_back(gasdata.index_offset / 3);
				...
				for (auto& primitives : meshs.primitives) {
					...
					for (int i = 0; i < indices.size() / 3; i++) {
						...
						for (int j = 0; j < 3; j++) {
							...
							scenedata.normals.push_back(norm[j]);
							scenedata.texcoords.push_back(texc[j]);
							scenedata.indices.push_back(scenedata.indices.size());
						}
